...
extension TMDBClient {
    ...
    */
    ...
    private func getRequestToken( completionHandlerForToken: @escaping (_ success: Bool, _ requestToken: String?, _ errorString: String?) -> Void) {
        ...
        /* 2. Make the request */
       let _ = taskForGETMethod(Methods.AuthenticationTokenNew, parameters: parameters){
            (results, error) in
        ...
        /* 3. Send the desired value(s) to completion handler */
            if let error = error {
                ...
                if let requestToken = results?[TMDBClient.JSONResponseKeys.RequestToken] as? String {
                    ...
                    completionHandlerForToken(false, nil, "Login failed. (Token request)")
                }
            }
        }
    ...
    private func getSessionID(_ requestToken: String?, completionHandlerForSession:@escaping (_ success: Bool,_ sessionID: String?,_ errorString: String?) -> Void) {
        ...
        let _ = taskForGETMethod(Methods.AuthenticationSessionNew, parameters: parameters as [String : AnyObject]){
            ...
            if let error = error {
                ...
                print(error)
                completionHandlerForSession(false,nil,"Login failed (Session ID. 1)")
            }else {
                if let sessionID = results?[TMDBClient.JSONResponseKeys.SessionID] as? String {
                    ...
                    completionHandlerForSession(false, nil, "Login failed (Session ID. 2)")
                }
            }
        }    }
    ...
    private func getUserID(_ completionHandlerForUserID: @escaping (_ success: Bool, _ userID: Int?, _ errorString: String?) -> Void) {
        ...
        /* 2. Make the request */
        let _ = taskForGETMethod(Methods.Account, parameters: parameters as [String : AnyObject]){
            (results, error) in
            
            /* 3. Send the desired value(s) to completion handler */
            if let error = error {
                print(error)
                completionHandlerForUserID(false, nil, "Login failed (User ID. 1)")
            }else {
